...
def KL_divergence(total_param, summary_param):
    valid_param = filter(lambda f:
            total_param[f]['sigma']>0.01,total_param.keys())
...
def extend_param(summary_param,total_param):
    param_add = set(total_param.keys()).difference(set(summary_param.keys()))
    def add2dict(f):
        summary_param[f]={'prob':0.0001,
                          'mu':0,
                          'sigma':0.01}
    map(add2dict,param_add)
...
def get_summary(feature_file,sentiment_file,sentence_file,output_file):
    ...
    #initial summary
        ...
        summary_param = extend_param(summary_param,total_param)
